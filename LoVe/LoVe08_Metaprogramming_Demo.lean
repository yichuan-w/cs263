...
/- ## Lean's Compilation Flow

There is a great book on [Metaprogramming in Lean](https://leanprover-community.github.io/lean4-metaprogramming-book/)
which I highly recommend for learning more about Lean's metaprogramming framework.
Here is a basic overview:

       Parse          Macro            Elab      Eval / Compile
String -----> Syntax -------> Syntax  ------> Expr ------> C code
                 ↖            ↙
                    (can loop)


When metaprogramming, we can hook in at any of these stages.

1. **Parsing** applies syntax rules to an input string to produce a syntax tree.
   We can define custom syntax rules to extend the input language.

2. **Macro expansion** applies macros to a syntax tree to produce another syntax tree.

3. **Elaboration** applies elaboration rules to a syntax tree to produce an expression.
   Expressions (`Expr`) are the internal representation of terms in Lean.
   When elaborating, you have access to the current context and goal.
...
def traceGoals (n: ℕ) : TacticM Unit :=
  do
    for _ in [:n] do
      logInfo m!"Hello from my custom tactic!"
