...
theorem term_irrel {a : Type} (h₁ h₂ : a) :
    h₁ = h₂ :=
  by sorry
...
abbrev upairSet { α : Type } (ab : α × α) : Set α := {ab.1, ab.2}

instance UPair.Setoid (α : Type) : Setoid (α × α) :=
{ r := fun ab₁ ab₂ ↦ upairSet ab₁ = upairSet ab₂
  iseqv :=
    { refl  := by simp
      symm  := by aesop
      trans := by aesop } }
