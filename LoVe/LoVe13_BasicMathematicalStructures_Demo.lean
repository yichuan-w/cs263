...
-- register the zero and add with the appropriate type classes
instance : Zero Int2 := ⟨ Int2.zero ⟩
instance : Add Int2  := ⟨ Int2.add ⟩
-- neg is id
instance : Neg Int2  := ⟨ id ⟩

-- Note: we are "one-shotting" a typeclass instance that is composed of smaller typeclasses
-- (go check!). You might do this more compositionally.
instance Int2.AddGroup : AddGroup Int2 :=
  {
    -- these are filled in by default thanks to the typeclass instances above
    -- add            := Int2.add
    -- zero           := Int2.zero
    -- neg            := fun a ↦ a
    add_assoc      :=
    ...
    neg_add_cancel :=
      by
        ...
        cases a <;>
          rfl
    -- these Rec things synthesize the correct impls of multiplication by ℕ/ℤ based on
    -- typeclass instances
    nsmul         := nsmulRec
    zsmul         := zsmulRec
        }

-- This **does** get you the parent instances
#check (inferInstance : AddMonoid Int2)
...
-- note: different algebraic requirements on sum due to quotient
#eval List.sum [2, 3, 4]
