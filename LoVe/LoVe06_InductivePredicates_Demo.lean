...
inductive Even : ℕ → Prop where
  ...
  -- | add_two (k: ℕ) (ke: Even k): Even (k + 2)
...
theorem Even_4 : Even 4 :=
  ...
  -- alternatively
  Even.add_two _ (Even.add_two _ Even.zero)
...
The inductive version is also an inductive type! No junk, etc.
We can inspect the details of the proof! (Not possible with Props)
-/


example (n: ℕ) (ne: Even n): ¬Even (n+1) := by
  induction ne
  . intro h
    cases h
  . intro h
...
-- proving the same thing with evenRec is possible,
-- but requires induction by n, and some extensions
-- to induction we don't know about yet.
...
This is silly, but the technique of inductive predicates for modeling transition systems
 is the same one we'll use to prove stuff about an imperative programming language.
...
example: 15–0  ↝ 30–0  := Step.serv_15_30 0
example: 15–99 ↝ 30–99 := Step.serv_15_30 99
...
-- I need BEq to even compare α's... I don't need that above.
def starRec {α : Type} [BEq α] (R : α → α → Bool) :
  α → α → Bool :=
  fun a b =>
    if R a b then true else
...
-- eq and iff are the same for Props!
#check funext
#check propext
